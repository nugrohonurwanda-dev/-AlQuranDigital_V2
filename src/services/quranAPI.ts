...
const UTHMANI_BASE_URL = 'https://api.quran.com/api/v4/quran/verses/uthmani';
...
export interface Verse {
  ...
  verse_number: number;
  verse_key: string;
  text_uthmani: string;          // teks bersih — untuk rendering utama
  text_uthmani_tajweed: string;  // teks dengan HTML tajweed — untuk warna saja
  text_latin?: string;
  translation?: string;
...
export const QuranAPI = {
  ...
  // Fetch text_uthmani (bersih) dan text_uthmani_tajweed (HTML) sekaligus
  // dengan dua request paralel supaya tidak nambah latency
  async getVerses(chapterNumber: number, page = 1, perPage = 50): Promise<Verse[]> {
    const [tajweedRes, uthmaniRes] = await Promise.all([
      fetch(`${TAJWEED_BASE_URL}?chapter_number=${chapterNumber}&page=${page}&per_page=${perPage}`),
      fetch(`${UTHMANI_BASE_URL}?chapter_number=${chapterNumber}&page=${page}&per_page=${perPage}`),
    ]);
    ...
    // Buat map text_uthmani bersih berdasarkan verse_key untuk lookup cepat
    ...
    return tajweedData.verses.map((verse: Verse, index: number): Verse => {
      const verseKey = verse.verse_key ?? `${chapterNumber}:${index + 1}`;
      return {
        id: verse.id ?? `${chapterNumber}-${index + 1}`,
        verse_number: verse.verse_number ?? index + 1,
        verse_key: verseKey,
        // text_uthmani bersih dari endpoint uthmani — fallback ke tajweed jika gagal
        text_uthmani: uthmaniMap[verseKey] ?? verse.text_uthmani ?? '',
        // text_uthmani_tajweed berisi HTML tag — hanya untuk data warna
  ...
  getRecitations(): Reciter[] {
    return [
      ...
      { id: '05', name: 'Misyari Rasyid Al-Afasi', folder_name: 'Misyari-Rasyid-Al-Afasi' },
    ];
  },
  ...
  getAudioUrl(reciterId: string, chapterNumber: number, verseNumber: number): string {
    ...
    return `https://cdn.equran.id/audio-partial/${folder}/${surah}${ayat}.mp3`;
  },
